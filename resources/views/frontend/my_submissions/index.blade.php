            ...
            <div class="card rounded-0">
                <div class="card-header">
                    <h5 class="card-title">
                        ...
                        <a href="{{ route('frontend.forms.index') }}" class="btn btn-primary float-md-right btn-sm" title="Back To My Forms">
                            ...
                            <tbody>
                                @foreach($submissions as $submission)
                                    <tr>
                                        ...
                                        <td>
                                            <a href="{{ route('frontend.my-submissions.show', [$submission->id]) }}" class="btn btn-primary btn-sm" title="View submission">
                                                <i class="fa fa-eye"></i> View
                                            </a> 

                                            @if($submission->form->allowsEdit())
                                                <a href="{{ route('frontend.my-submissions.edit', [$submission->id]) }}" class="btn btn-primary btn-sm" title="Edit submission">
                                                    <i class="fa fa-pencil"></i> 
                                                </a> 
                                            @endif

                                            {{-- <form action="{{ route('frontend.my-submissions.destroy', [$submission]) }}" method="POST" id="deleteSubmissionForm_{{ $submission->id }}" class="d-inline-block">
                                                @csrf 
                                                @method('DELETE')
